...
export function useResource<T>(url: string, key?: string) {
  ...
  const load = useCallback(async (signal?: AbortSignal) => {
    ...
    const current = () => !signal?.aborted && requestId === revision.current;
    setLoading(true); setError("");
    try { const result = await jsonRequest(url, { signal }); if (current()) setData(key ? result[key] : result); }
    catch (e) { if (current()) setError(e instanceof Error ? e.message : "Unable to load records."); }
