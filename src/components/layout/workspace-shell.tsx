    ...
    <div className="min-h-screen lg:flex">
      <aside className="border-b border-slate-200 bg-white p-5 lg:min-h-screen lg:w-64 lg:border-b-0 lg:border-r">
        ...
        <nav className="space-y-1 text-sm" aria-label="Primary navigation">
          ...
          {[['Leads', '/leads'], ['Clients', '/clients'], ['Projects', '/projects'], ['Finance', '/finance'], ['Infrastructure', '/infrastructure'], ['Demo environments', '/infrastructure/demos'], ['Search', '/workspace/search']].map(([item, href]) => <a className="block rounded-lg px-3 py-2 text-slate hover:bg-slate-50 hover:text-ink" href={href} key={item}>{item}</a>)}
        </nav>
      </aside>
