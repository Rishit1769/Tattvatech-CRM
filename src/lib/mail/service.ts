...
export async function sendEmail(input: SendEmailInput) {
  ...
  try { await mailer.sendMail({ from: env.SMTP_FROM_EMAIL ? { name: env.SMTP_FROM_NAME, address: env.SMTP_FROM_EMAIL } : env.SMTP_FROM, to: input.recipientEmail, subject: input.subject, html: input.html }); await prisma.emailEvent.update({ where: { id: event.id }, data: { status: "SENT", sentAt: new Date() } }); return { status: "SENT" as const, eventId: event.id }; } catch (error) { const message = error instanceof Error ? error.message : "SMTP delivery failed"; await prisma.emailEvent.update({ where: { id: event.id }, data: { status: "FAILED", failureMessage: message } }); return { status: "FAILED" as const, eventId: event.id, error: message }; }
}
