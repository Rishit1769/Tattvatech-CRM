...
export async function GET() { try { await requirePermission("follow_up.view"); const followUps = await prisma.followUp.findMany({ where: { status: "OPEN" }, include: { lead: true, client: true }, orderBy: { dueAt: "asc" }, take: 100 }); return NextResponse.json({ followUps }); } catch (error) { return error instanceof Error && error.message === "UNAUTHENTICATED" ? apiError("Authentication required", 401) : error instanceof Error && error.message === "FORBIDDEN" ? apiError("Forbidden", 403) : apiError("Unable to load follow-ups", 500); } }
