...
export async function GET(request: Request) {
  try {
    ...
    const projects = await prisma.project.findMany({ where: { AND: [await projectVisibilityWhere(user.id), assignedOnly ? { members: { some: { userId: user.id, status: "ACTIVE" } } } : {}, lifecycle ? { lifecycleStatus: lifecycle } : {}, payment ? { paymentStatus: payment } : {}, search ? { OR: [{ name: { contains: search } }, { projectCode: { contains: search } }, { client: { name: { contains: search } } }] } : {}] }, include: { client: true, family: true, department: true, owner: { select: { fullName: true } }, manager: { select: { fullName: true } }, technicalOwner: { select: { fullName: true } }, deployments: { where: { status: "ACTIVE" }, orderBy: { deployedAt: "desc" }, take: 1 }, tasks: { select: { status: true } }, _count: { select: { tasks: true, milestones: true, members: true, modules: true } } }, orderBy: { updatedAt: "desc" }, take: 100 });
    return NextResponse.json({ projects: projects.map((project) => { const activeTasks = project.tasks.filter((task) => task.status !== "CANCELLED"); const progress = activeTasks.length ? Math.round(activeTasks.filter((task) => task.status === "DONE").length / activeTasks.length * 100) : null; return { ...project, dealValue: project.dealValue?.toString() ?? null, progress }; }) });
  } catch (error) {
