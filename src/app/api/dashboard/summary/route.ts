...
export async function GET() {
  try {
    ...
    const [activeLeads, pipeline, activeProjects, received, openIncidents, runningDemos, planningProjects, developmentProjects, testingProjects, deployedProjects] = await Promise.all([
      ...
      prisma.project.findMany({ where: { AND: [projectScope, { lifecycleStatus: "DEPLOYED", deployments: { some: { status: "ACTIVE" } } }] }, include: { client: true, technicalOwner: { select: { fullName: true } }, deployments: { where: { status: "ACTIVE" }, orderBy: { deployedAt: "desc" }, take: 1 } }, orderBy: { updatedAt: "desc" }, take: 50 }),
    ]);
    return NextResponse.json({ activeLeads, pipelineValue: pipeline._sum.expectedValue?.toString() ?? null, activeProjects, received: received._sum.amount?.toString() ?? null, openIncidents, runningDemos, lifecycle: { planning: planningProjects, development: developmentProjects, testing: testingProjects, deployed: deployedProjects.length }, deployedProjects });
  } catch (error) {
