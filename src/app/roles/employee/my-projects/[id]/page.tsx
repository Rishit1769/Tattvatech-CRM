...
export default async function EmployeeProjectDetail({ params }: { params: Promise<{ id: string }> }) { const user = await getCurrentUser(); if (!user) redirect("/login"); if (!user.permissions.includes("project.view_assigned")) redirect("/dashboard"); const { id } = await params; return <WorkspaceShell user={user}><AssignedProjectDetail id={id} role="employee" /></WorkspaceShell>; }
